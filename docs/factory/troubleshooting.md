...
When a live Pi run has a decision UI available, recoverable runtime problems can pause as `DECISION_REQUIRED / decision-runtime` before Factory writes a terminal failure. The prompt names the phase, problem category, artifact references, and choices such as `I fixed it; retry this phase`, `Revise with my guidance`, or `Stop and preserve the failure`. Optional recovery notes are recorded in `decisions.jsonl`, `recovery-checkpoint.json` records the phase context, and the run retries only the affected phase when that retry is safe. This covers discovery, planning, implementation, integration, verification-planning, review, approval, landing, and post-landing verification gates. Landing guard, execution, and post-landing verification can consume `revise` feedback by rerunning the affected planner or repair step; `stop` keeps the PR fallback and exhausted attempts preserve the blocked/PR path.
