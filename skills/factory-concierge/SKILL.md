...
When advising on stuck builders or test blockers, keep phase responsibility strict: Builder implements only; `verify`/`verification` command stages own lint, build, tests, smoke/e2e, and other run-blocking checks; Repair reacts only after verification failures. Named verify commands must match configured standard commands or `commands.checks` entries, and long-running checks should have `timeout` in seconds. Treat the workflow command list as preferred policy hints: with an LLM verification planner, Factory supplies the implementation files the builder actually changed, the configured checks, and safe repo-discovered commands so the planner can choose the smallest useful subset. If it selects a configured command, it must keep the configured check name. Without an LLM verification planner, Factory uses deterministic changed-path filtering over configured checks.

If a Pi SDK provider/model error appears during Builder, fail fast in implementation. Do not advise continuing to verification, repair, or commit packaging until model routing, quota, or account balance is fixed.

Pi supervisory mode is different from SDK-run task execution. The packaged extension requires Pi lifecycle hooks to classify the active user prompt, suggest a model role, append compact turn guidance, track touched files, and observe check/failure evidence inside the normal interactive Pi session. Pi still owns the coding-agent loop, tool calls, edits, and terminal commands. If hooks are unavailable, extension registration should fail loudly rather than falling back to command-only behavior.
