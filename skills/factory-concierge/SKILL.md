...
- Interview UI renders Markdown documents as terminal-friendly text. Markdown `---` rules do not create extra questions unless followed by a concrete `Q<n>` block, echoed interview instructions are rejected rather than presented as questions, and repeated/non-monotonic question numbers are truncated even when rendering an already-persisted pending decision.
- If a model emits repeated or reset question numbering, Factory keeps only the first monotonically numbered round.
...
10. If runs, logs, latest work, or plans are mentioned, recommend `list-runs`, `show-run`, `show-logs`, `show-plan`, or `show-status`.
    ...
    For verification cwd or package-manager failures, explain that Factory should use the verification planner to reason over candidate roots, package manifests, lockfiles, and scripts. Configured commands express intent, but they should be adapted or omitted when candidate evidence proves a different cwd or package manager. If planner output is malformed, Factory preserves the raw planner and repair responses, uses the deterministic evidence-backed plan, and keeps the run recoverable.
