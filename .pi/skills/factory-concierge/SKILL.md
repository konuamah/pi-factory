...
- Interview Markdown is rendered as terminal-friendly text. Horizontal rules only separate questions when followed by a concrete `Q<n>` block; echoed role/instruction documents are rejected instead of shown as user questions.
- Repeated or reset question numbering from a model response is truncated to the first monotonically numbered round before display.
