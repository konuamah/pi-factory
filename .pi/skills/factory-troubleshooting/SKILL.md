...
Completion is gated by evidence, not by the word `merged`: implementation tasks cannot be blocked, verification must pass, final-merge.json must say `merged`, and the candidate must contain a non-generated product-file diff. Passing lint/build with only `.factory/` or dependency artifacts leaves the run `BLOCKED` as `no-product-changes`.
...
When a monorepo package is selected for dependency setup, treat it as a package root only. Discovery, planning, and implementation must continue from the Git worktree root so sibling packages remain visible. Inspect `dependency_strategy.selected` and compare `executionRoot` with `dependencyRoot` before diagnosing missing files.
